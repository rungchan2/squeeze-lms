  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      try {
        ...
        if (journeyError) {
          throw new Error(`여정 데이터를 가져오는 중 오류 발생: ${journeyError.message}`);
        }
        
        const journeyId = journeyData.id;
        
        // 1. 현재 여정의 미션 인스턴스 가져오기
        const { data: missionInstances, error: missionsError } = await supabase
        ...
        if (missionsError) {
          throw new Error(`미션 데이터를 가져오는 중 오류 발생: ${missionsError.message}`);
        }
        ...
        if (postsError) {
          throw new Error(`게시물 데이터를 가져오는 중 오류 발생: ${postsError.message}`);
        }
        ...
        completedMissionInstances.forEach(mission => {
          // 해당 미션에 대한 게시물 찾기
          const relatedPosts = posts.filter(post => post.mission_id === mission.mission_id);
        ...
        if (usersError) {
          throw new Error(`사용자 데이터를 가져오는 중 오류 발생: ${usersError.message}`);
        }
        ...
        const completedMissionsCount = missionInstances.filter(m => m.status === 'completed').length;
        const submittedMissionsCount = missionInstances.filter(m => m.status === 'submitted').length;
        const inProgressMissionsCount = missionInstances.filter(m => m.status === 'in_progress').length;
        const notStartedMissionsCount = missionInstances.filter(m => m.status === 'not_started').length;
          ...
          <StatsContainer>
            ...
            <StatsDetailsContainer>
              <StatItem>
                ...
                <Text variant="body" fontWeight="bold">{submissionStats.totalMissions}개</Text>
              </StatItem>
              <StatItem>
            ...
            {leaderboardUsers.map((user) => (
              ...
              >
                ...
                <div className="user-info">
                  <ProfileImage profileImage={user.profile_image || ""} width={40} />
                  <div className="user-details">
                    <Text variant="body" fontWeight="bold">
                      {user.first_name} {user.last_name}
                      {user.isCurrentUser && <span className="current-user-badge">나</span>}
                    </Text>
...
const LeaderboardItem = styled.div<{ rank: number; isCurrentUser: boolean }>`
  ...
  background-color: ${props => 
    props.isCurrentUser ? 'var(--primary-50)' :
    props.rank === 1 ? 'var(--gold-50)' : 
    props.rank === 2 ? 'var(--silver-50)' : 
    props.rank === 3 ? 'var(--bronze-50)' : 
    'var(--white)'
  };
