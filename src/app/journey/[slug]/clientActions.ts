...
export async function getUserPointsByJourneyId(journeyId: string | null) {
  ...
  try {
    ...
    if (!journeyWeeks || journeyWeeks.length === 0) {
      console.log(
        "[getUserPointsByJourneyId] 해당 여정에 주차가 없음:",
        journeyId
      );
      return { data: [], error: null };
    }
    ...
    console.log(
      "[getUserPointsByJourneyId] 미션 인스턴스 ID:",
      missionInstanceIds
    );
    ...
    console.log(
      "[getUserPointsByJourneyId] 필터링된 포인트 수:",
      userPoints?.length || 0
    );
...
export async function getJourneyWeeklyStats(journeyId: string) {
  try {
    ...
    const weeklyStats = weeks.map((week) => {
      ...
      const submittedMissions = weekMissionIds.filter((id) =>
        completedMissionInstanceIds.has(id)
      ).length;
